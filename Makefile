...
MOBILEGLUES_CPP_DIR   ?= $(SOURCEDIR)/Natives/external/MobileGlues/src/main/cpp
ifeq (,$(wildcard $(MOBILEGLUES_CPP_DIR)/CMakeLists.txt))
MOBILEGLUES_CPP_DIR   := $(SOURCEDIR)/Natives/external/MobileGlues/MobileGlues-cpp
endif
...
java:
	...
	$(MAKE) -C JavaApp -j$(JAVA_JOBS) BOOTJDK=$(BOOTJDK) JAVA_RELEASE=$(JAVA_RELEASE)
...
dep_mg:
	...
	if [ -f "$(MOBILEGLUES_CPP_DIR)/libraries/ios/libspirv-cross-c-shared.0.dylib" ]; then \
		cp $(MOBILEGLUES_CPP_DIR)/libraries/ios/libspirv-cross-c-shared.0.dylib $(WORKINGDIR)/libspirv-cross-c-shared.0.dylib; \
	fi
