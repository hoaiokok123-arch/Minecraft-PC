...
- (void)installModpackFromDetail:(NSDictionary *)modDetail atIndex:(NSUInteger)selectedVersion {
    [self installProjectFromDetail:modDetail atIndex:selectedVersion];
}

- (void)installProjectFromDetail:(NSDictionary *)modDetail atIndex:(NSUInteger)selectedVersion {
    // Pass details to LauncherNavigationController
    NSDictionary* userInfo = @{
        @"detail": modDetail,
        @"index": @(selectedVersion)
    };
    NSString *notificationName = [modDetail[@"projectType"] isEqualToString:@"modpack"] ?
        @"InstallModpack" : @"InstallModrinthProject";
