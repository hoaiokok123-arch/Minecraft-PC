...
- (void)viewDidLoad
{
    ...
    BOOL hasLiquidGlass = UIKitHasLiquidGlass();
...
- (void)receiveNotification:(NSNotification *)notification {
    BOOL installModpack = [notification.name isEqualToString:@"InstallModpack"];
    BOOL installProject = [notification.name isEqualToString:@"InstallModrinthProject"];
    if (!installModpack && !installProject) {
        return;
    }
    [self setInteractionEnabled:NO forDownloading:YES];
    self.task = [MinecraftResourceDownloadTask new];
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        if (installProject) {
            [self.task downloadProjectFileFromAPI:notification.object detail:userInfo[@"detail"] atIndex:[userInfo[@"index"] unsignedLongValue]];
        } else {
