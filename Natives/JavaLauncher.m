...
NSString *getSelectedLWJGLJarName(NSString *librariesPath) {
    NSString *version = [PLProfiles resolveKeyForCurrentProfile:@"lwjglVersion"];
    if (version.length == 0 || [version isEqualToString:@"(default)"]) {
        version = @"3.3.3";
    }

    NSString *versionedJar = [NSString stringWithFormat:@"lwjgl-%@.jar", version ?: @"3.3.3"];
    if ([fm fileExistsAtPath:[librariesPath stringByAppendingPathComponent:versionedJar]]) {
        return versionedJar;
    }
    if ([version isEqualToString:@"3.3.1"]) {
        NSLog(@"[JavaLauncher] Requested LWJGL 3.3.1, but %@ is missing. Falling back to bundled default.", versionedJar);
    }
    if ([fm fileExistsAtPath:[librariesPath stringByAppendingPathComponent:@"lwjgl-3.3.3.jar"]]) {
        return @"lwjgl-3.3.3.jar";
    }
    return @"lwjgl.jar";
}

NSString *getLauncherClasspath(NSString *librariesPath, NSString *extraJar) {
    NSString *selectedLWJGLJar = getSelectedLWJGLJarName(librariesPath);
    NSArray<NSString *> *files = [[fm contentsOfDirectoryAtPath:librariesPath error:nil]
        sortedArrayUsingSelector:@selector(localizedCaseInsensitiveCompare:)];
    NSMutableArray<NSString *> *classpath = [NSMutableArray new];
    for (NSString *file in files) {
        if (![file hasSuffix:@".jar"]) {
            continue;
        }
        BOOL isLWJGLBundle = [file isEqualToString:@"lwjgl.jar"] ||
            ([file hasPrefix:@"lwjgl-"] && [file hasSuffix:@".jar"]);
        if (isLWJGLBundle && ![file isEqualToString:selectedLWJGLJar]) {
            continue;
        }
        [classpath addObject:[librariesPath stringByAppendingPathComponent:file]];
    }
    if (extraJar.length > 0) {
        [classpath addObject:extraJar];
    }
    NSLog(@"[JavaLauncher] LWJGL classpath is %@", selectedLWJGLJar);
    return [classpath componentsJoinedByString:@":"];
}
...
int launchJVM(NSString *username, id launchTarget, int width, int height, int minVersion) {
    ...
    NSString *classpath = getLauncherClasspath(librariesPath, launchJar ? launchTarget : nil);
    margv[++margc] = "-cp";
