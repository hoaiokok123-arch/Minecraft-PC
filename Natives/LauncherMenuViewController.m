...
+ (LauncherMenuCustomItem *)vc:(UIViewController *)vc {
    LauncherMenuCustomItem *item = [[LauncherMenuCustomItem alloc] init];
    item.title = [vc title];
    item.imageName = [(id)vc imageName];
...
- (void)viewDidLoad {
    ...
    self.options = @[
        ...
        [LauncherMenuCustomItem vcClass:ManagerViewController.class],
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (origImage) {
        ...
        UIImage *image = [renderer imageWithActions:^(UIGraphicsImageRendererContext*_Nonnull myContext) {
            CGFloat maxSize = 32;
            CGFloat scaleFactor = MIN(maxSize / origImage.size.width, maxSize / origImage.size.height);
            CGSize size = CGSizeMake(origImage.size.width * scaleFactor, origImage.size.height * scaleFactor);
            [origImage drawInRect:CGRectMake((40 - size.width) / 2, (40 - size.height) / 2, size.width, size.height)];
        }];
