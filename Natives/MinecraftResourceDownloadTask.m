...
- (NSURLSessionDownloadTask *)createDownloadTask:(NSString *)url size:(NSUInteger)size sha:(NSString *)sha altName:(NSString *)altName toPath:(NSString *)path success:(void (^)())success {
    ...
    if (fileExists && [self checkSHA:sha forFile:path altName:altName]) {
        if (success) success();
        return nil;
    } else if (self.requiresMinecraftAccess && ![self checkAccessWithDialog:YES]) {
        return nil;
    }
...
- (void)downloadModpackFromAPI:(ModpackAPI *)api detail:(NSDictionary *)modDetail atIndex:(NSUInteger)selectedVersion {
    self.requiresMinecraftAccess = NO;
    [self prepareForDownload];
    ...
    NSURLSessionDownloadTask *task = [self createDownloadTask:url size:size sha:sha altName:nil toPath:packagePath success:^{
        ...
        [self dropInitialProgressUnit];
    }];
    [task resume];
}

- (NSString *)gameDirectoryForSelectedProfile {
...
- (NSString *)folderNameForProjectType:(NSString *)projectType {
    ...
    return folders[projectType] ?: @"downloads";
}
...
- (void)downloadProjectFileFromAPI:(ModpackAPI *)api detail:(NSDictionary *)modDetail atIndex:(NSUInteger)selectedVersion {
    ...
    NSString *projectType = modDetail[@"projectType"];
    if ([projectType isEqualToString:@"modpack"] || [[fileName pathExtension] isEqualToString:@"mrpack"]) {
        [self downloadModpackFromAPI:api detail:modDetail atIndex:selectedVersion];
        return;
    }
    ...
    NSUInteger size = [modDetail[@"versionSizes"][selectedVersion] unsignedLongLongValue];
    NSString *sha = modDetail[@"versionHashes"][selectedVersion];
    NSString *destName = [self safeFileName:fileName fallbackURL:url];
    BOOL installDisabled = [modDetail[@"installDisabled"] boolValue];
    if (installDisabled && ![destName hasSuffix:@".disabled"]) {
        destName = [destName stringByAppendingString:@".disabled"];
    }
    NSString *replacePath = [modDetail[@"replaceFilePath"] isKindOfClass:NSString.class] ?
        [modDetail[@"replaceFilePath"] stringByStandardizingPath] : nil;
    BOOL replacingExistingMod = replacePath.length > 0;
    ...
    if (replacingExistingMod) {
        NSString *tempName = [NSString stringWithFormat:@"%@.%@.download", destName, [[NSUUID UUID] UUIDString]];
        downloadPath = [NSTemporaryDirectory() stringByAppendingPathComponent:tempName];
    }

    NSURLSessionDownloadTask *task = [self createDownloadTask:url size:size sha:sha altName:destName toPath:downloadPath success:^{
        if (replacingExistingMod) {
            ...
            BOOL backupMoved = NO;
            if ([replacePath isEqualToString:destPath] && [fm fileExistsAtPath:replacePath]) {
                backupPath = [NSTemporaryDirectory() stringByAppendingPathComponent:
                    [NSString stringWithFormat:@"%@.%@.backup", destName, [[NSUUID UUID] UUIDString]]];
                if (![fm moveItemAtPath:replacePath toPath:backupPath error:&error]) {
                    [self finishDownloadWithErrorString:error.localizedDescription];
                    return;
                }
                backupMoved = YES;
            } else if ([fm fileExistsAtPath:destPath] && ![fm removeItemAtPath:destPath error:&error]) {
                [self finishDownloadWithErrorString:error.localizedDescription];
                return;
            }

            if (![fm moveItemAtPath:downloadPath toPath:destPath error:&error]) {
                if (backupMoved) {
                    [fm moveItemAtPath:backupPath toPath:replacePath error:nil];
                }
                [self finishDownloadWithErrorString:error.localizedDescription];
                return;
            }
            ...
            } else if (![replacePath isEqualToString:destPath] && [fm fileExistsAtPath:replacePath]) {
                [fm removeItemAtPath:replacePath error:nil];
            }
        }
        [self markDownloadFinished];
        dispatch_async(dispatch_get_main_queue(), ^{
            [NSNotificationCenter.defaultCenter postNotificationName:@"ModManagerModFileDidChange"
                object:self
                userInfo:@{@"path": destPath}];
        });
    }];
    [task resume];
}
